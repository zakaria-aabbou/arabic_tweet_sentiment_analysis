...
clf = MultinomialNB().partial_fit(X_train, y_train, classes=[0, 1])

# Une fois le modèle ajusté, il est évalué sur l'ensemble de données de test.
accuracy = clf.score(X_test, y_test)
print("Naivebayes accuracy: %.2f%%" % (accuracy * 100.0))

with open("models/arabic_sentiment_naivebayes_tokenizer.pickle", "wb") as f:
    pickle.dump(tf_vec, f)
with open("models/arabic_sentiment_naivebayes.pickle", "wb") as f:
    pickle.dump(clf, f)
